<template>
  <div class="manage-page">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button type="primary" @click="batchExport" :disabled="selectedAchievements.length === 0">
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <div class="action-buttons">
              ...
              <el-button size="small" type="primary" @click="editAchievement(row)">
    ...
    >
      <div v-if="editAchievementData" class="edit-form">
        <div class="achievement-info">
          <h4>{{ editAchievementData.title }}</h4>
          <p>提交人：{{ editAchievementData.submitter_name }}</p>
        </div>
...
const fetchAchievements = async () => {
  ...
  try {
    ...
    const response = await getTeamAchievements(params)
...
const editAchievement = (row) => {
  ...
  editForm.is_public = row.is_public || false
...
const submitEdit = async () => {
  ...
  await editFormRef.value.validate(async (valid) => {
    if (valid) {
      editing.value = true
      try {
        await updateAchievementInfo(editAchievementData.value.achievement_id, editForm)
...
const batchExport = async () => {
  try {
    ...
    await exportAchievements({ achievement_ids: achievementIds, format: 'excel' })
